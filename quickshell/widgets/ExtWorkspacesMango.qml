...
RowLayout {
    ...
    property var workspaceIds: {
        ...
        for (const ws of projection.windowsets) {
            if (ws.shouldDisplay && !ids.includes(ws.name)) ids.push(ws.name)
        }
    ...
    Repeater {
        ...
        Rectangle {
            ...
            property var ws: {
                const found = workspacesRow.projection.windowsets.find(w => w.name === wsId)
                console.log("wsId:", wsId, typeof wsId, "-> found:", found ? found.name : "NONE", found ? typeof found.name : "")
                return found
            }
            property bool isActive: {
                console.log("isActive eval for", wsId, "ws:", ws, "ws.active:", ws?.active)
                return ws?.active ?? false
            }
            ...
            Component.onCompleted: {
                for (const w of projection.windowsets)
                    console.log("name:", w.name, "active:", w.active, "shouldDisplay:", w.shouldDisplay)
            }
        }
    }
}
